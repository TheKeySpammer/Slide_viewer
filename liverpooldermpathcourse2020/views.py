...
from .models import Liverpooldermpathcourse2020, Annotation
...
class Openslides:
    ...
    def insertslide(cls, key, sl):
        opts = {'tile_size': settings.DEEPZOOM_TILE_SIZE,
...
def slide(request, slide_id):
    try:
        s = Liverpooldermpathcourse2020.objects.get(pk=slide_id)
    except Liverpooldermpathcourse2020.DoesNotExist:
        raise Http404
    return render(request, 'liverpooldermpathcourse2020/slide.html', {'Slide': s})
...
def get_slide(slide_id):
    if slide_id not in Openslides._slides:
        s = Liverpooldermpathcourse2020.objects.get(pk=slide_id)
        load_slide(s.pk, path.join(
            settings.HISTOSLIDE_SLIDEROOT, str(s.UrlPath)))
...
def dzi(request, slug):
    ...
    try:
        slug = int(slug)
        resp = HttpResponse(get_deepzoom(slug).get_dzi(
            slideformat), content_type='application/xml')
        return resp
    except KeyError:
        # Unknown slug
        raise Http404
...
def add_annotation(request):
    if (request.method == "POST"):
        data = request.POST
        slide = Liverpooldermpathcourse2020.objects.get(
            pk=int(data['slideId']))
