...
from .models import Jclinpathtest
...
def openslide_file(self):
    ...
    return format_html(
       ...
       ("%s%s" % (settings.DEFAULT_DOMAIN+ '/jclinpathtest/slide/', slide_id)),
       'View Case',
    )
...
def generate_iframe(self):
    ...
    return format_html('{} <div id="iframe_dialog"><textarea class="iframe_code" readonly rows="4" cols="91">{}</textarea></div>',
        ...
        mark_safe('<body style="margin:0px;padding:0px;overflow:hidden"><iframe src="'+settings.DEFAULT_DOMAIN+'/jclinpathtest/slide/'+str(slide_id)+'" frameborder="0" style="overflow:hidden;height:100%;width:100%" height="100%" width="100%"></iframe></body>'),
    )
...
def download_slide(self):
    ...
    return format_html(
       '<a href="{}" target="_blank">{}</a>',
       ("%s%s%s" % (settings.DEFAULT_DOMAIN+ '/jclinpathtest/slide/', slide_id, '/download')),
       'Download Case',
    )
...
def delete_slide(self):
    ...
    return format_html(
       '<a href="{}" target="_blank">{}</a>',
       ("%s%s%s" % (settings.DEFAULT_DOMAIN+ '/jclinpathtest/slide/', slide_id, '/delete')),
       'Delete Case',
    )
delete_slide.short_description = 'Delete Case'


class JclinpathtestAdmin(admin.ModelAdmin):
        ...
        for root, dirs, files in walk(path.join(settings.HISTOSLIDE_SLIDEROOT), followlinks=True):
            for slide_file in files:
                if slide_file.lower().endswith(settings.SLIDEFILE_EXTENSIONS):
                    ...
                    try:
                        slide = Jclinpathtest.objects.get(UrlPath=str(urlnew))
                    except Jclinpathtest.DoesNotExist:
                        slide = Jclinpathtest(Name=path.splitext(slide_file)[0], ScannedBy=currentuser,
...
admin.site.register(Jclinpathtest, JclinpathtestAdmin)