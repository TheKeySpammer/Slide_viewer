...
from .models import Alopecia, Annotation
import base64
import re
...
def slide(request, slide_id):
    ...
    return render(request, 'alopecia/slide.html', {'Slide': s, 'Label': request.build_absolute_uri('/static/labels/'+path.basename(s.LabelUrlPath.url)), 'back_url': back_url, 'annotations': s.Annotations})
...
def get_group_list(request, slide_id):
    try:
        ...
        raise Http404
    groupId = s.Group
    listSlide = Alopecia.objects.filter(Group=groupId).order_by('Name')
