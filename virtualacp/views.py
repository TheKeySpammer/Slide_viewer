...
from .models import Virtualacp, Annotation
...
def slide(request, slide_id):
    try:
        s = Virtualacp.objects.get(pk=slide_id)
    except Virtualacp.DoesNotExist:
        raise Http404
    return render(request, 'virtualacp/slide.html', {'Slide': s})
...
def upload(req):
    if req.user.is_authenticated:
        if req.method == 'GET':
            ...
            slide = req.FILES['filepond']
            p = path.join(settings.HISTOSLIDE_SLIDEROOT, 'media/uploads/filepond/')
            with open(str(p)+slide.name, 'wb+') as dest:
